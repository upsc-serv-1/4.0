...
type HeatmapRow = {
   ...
   * value through callbacks ΓÇö keeping navigation logic intact.
   */
  displayLabel?: string;
...
export default function PyqAnalysisTab({ isEmbedded }: { isEmbedded?: boolean }) {
  ...
  const fetchPyqData = async (bypassCache = false) => {
    ...
    try {
      ...
      // Save to cache ΓÇö store enriched questions so _institutes/_explanations survive cache
      KVStore.setJson(cacheKey, {
        questions: mergedQs,
  ...
  const topicHeatmapRows = useMemo<HeatmapRow[]>(() => {
    return topTopics.map(topic => {
      ...
      return {
        key: `topic-${topic}`,
        label: topic,          // original topic ΓÇö used by callbacks
  ...
  const questionExportSummary = useMemo(() => {
    ...
    return {
      ...
      primaryHeatmapTitle: isSingleSubject && deepDiveSubject ? `${deepDiveSubject} Section Group ├ù Year Heatmap` : 'Subject ├ù Year Heatmap',
      primaryHeatmapLabel: isSingleSubject ? 'Section Group' : 'Subject',
      secondaryHeatmapTitle: isSingleSubject && deepDiveSubject ? `${deepDiveSubject} Micro Topic ├ù Year Heatmap` : 'Top 20 Topics ├ù Year Heatmap',
  ...
  const buildAnalysisExecutiveSummaryHtml = (selectedReports: Record<string, boolean>): string => {
    ...
    // Forecast data ΓÇö produced from currently filtered raw questions so the
    ...
    return buildPyqAnalysisSummaryHtml({
      ...
      forecastTitle: questionExportScope === 'selected_subject' && questionExportSubject
        ? `Forecast ΓÇö ${questionExportSubject} (Probable 2026 Topics)`
        : 'Forecast ΓÇö Probable 2026 Topics',
    });
  };
  ...
  const exportPdf = async (mode: ExportMode, subjectOverride?: string) => {
    ...
    const buildSubjectDeepDive = (subject: string) => {
      ...
      return `
        ...
        <h2>${esc(subject)} ΓÇö Deep Dive</h2>
    ...
    return (
      <View style={styles.blockGap}>
        <View style={styles.paletteRow}>
          ...
          >
            <Text style={[styles.paletteChipText, { color: '#fff', fontWeight: '800', fontSize: 11 }]}>≡ƒöä Dedup</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={{ backgroundColor: colors.primary + '08', borderRadius: 12, padding: 12, marginBottom: 12, borderWidth: 1, borderColor: colors.primary + '20' }}>
          ...
          <Text style={{ color: colors.textSecondary, fontSize: 11, fontWeight: '600', lineHeight: 17 }}>
            {'ΓÇó Tap a row label (left) to open deep-dive sections & micro-topics.\nΓÇó Tap any cell number to directly open those questions in Learn Mode.\nΓÇó Tap the ≡ƒôÜ question-bank icon on a row to open all questions for that topic.'}
          </Text>
        </View>
