...
export function MainsScreenInner() {
  ...
  const overrideWithParamsRef = useRef(false);
  ...
  useEffect(() => {
    AsyncStorage.getItem('@mains_qb_filters').then(val => {
      ...
      qbPersistLoaded.current = true;
      if (overrideWithParamsRef.current) {
        // If params arrived while we were loading, force save them now
        setSessionFilters(current => {
          if (current) AsyncStorage.setItem('@mains_qb_filters', JSON.stringify(current)).catch(() => {});
          return current;
        });
      }
    });
      ...
      const onBackPress = () => {
        ...
        if (currentScreen === 'detailed-question') {
          if (cameFromExternalRoute.current) {
            ...
            setCurrentScreen('hub');
            setDetailedQuestion(null);
            navigation.goBack(); // Return to PYQ
            return true;
          }
          setDetailedQuestion(null);
...
const fuzzyMatchSubject = (subjectFilter: string[], targetSubject: string) => {
  if (subjectFilter.length === 0) return true;
  return subjectFilter.some(sf => {
    const sfNorm = sf.trim().toLowerCase();
    const qNorm = (targetSubject || '').trim().toLowerCase();
    return sfNorm === qNorm || (sfNorm === 'ethics' && qNorm.includes('ethics'));
  });
};

const fuzzyMatchStandard = (filterArray: string[], target: string) => {
  if (filterArray.length === 0) return true;
  return filterArray.some(f => f.trim().toLowerCase() === (target || '').trim().toLowerCase());
};
...
function HierarchyModal({
    ...
    if (activeCategoryItems && activeCategoryItems.length > 0) {
      activeCategoryItems.forEach(item => {
        getItemPaths(item).forEach(path => {
          if (fuzzyMatchPaper(paperFilter, path.paper)) {
      ...
      questions?.forEach(q => {
        if (q && q.paper && fuzzyMatchPaper(paperFilter, q.paper)) {
  ...
  const localSectionOptions = React.useMemo(() => {
    ...
    if (activeCategoryItems && activeCategoryItems.length > 0) {
      activeCategoryItems.forEach(item => {
        getItemPaths(item).forEach(path => {
          const matchPaper = fuzzyMatchPaper(paperFilter, path.paper);
          const matchSubject = fuzzyMatchSubject(subjectFilter, path.subject);
    ...
    if (activeCategoryItems && activeCategoryItems.length > 0) {
      activeCategoryItems.forEach(item => {
        getItemPaths(item).forEach(path => {
          const matchPaper = fuzzyMatchPaper(paperFilter, path.paper);
          const matchSubject = fuzzyMatchSubject(subjectFilter, path.subject);
          const matchSection = fuzzyMatchStandard(sectionFilter, path.sectionGroup || '');
      ...
      questions?.forEach(q => {
        if (q) {
          const matchPaper = fuzzyMatchPaper(paperFilter, q.paper || '');
          const matchSubject = fuzzyMatchSubject(subjectFilter, q.subject || '');
          const matchSection = fuzzyMatchStandard(sectionFilter, getQuestionSection(q));
          if (matchPaper && matchSubject && matchSection) {
    ...
    if (activeCategoryItems && activeCategoryItems.length > 0) {
      activeCategoryItems.forEach(item => {
        getItemPaths(item).forEach(path => {
          ...
          if (matchPaper && matchSubject && matchSection) {
            ...
            const isStandardHierarchyCat = ['intro_conclusion', 'quotes', 'mnemonics', 'frameworks', 'ethics', 'keywords_hub', 'case_studies_hub', 'sc_judgments_hub', 'data_facts'].includes(currentCat || '');
            if (isStandardHierarchyCat) {
              if (path.microtopic && fuzzyMatchStandard(microtopicFilter, path.microtopic)) {
                if (path.subtopic) stSet.add(path.subtopic);
              }
            } else {
              const themeName = item.category === 'data_facts' ? item.metric : item.title;
              if (fuzzyMatchStandard([selectedMicrotopic], themeName || '')) {
                const subThemes = item.parsedSubThemes || splitSubThemes(item.context);
      ...
      questions?.forEach(q => {
        if (q) {
          const matchPaper = fuzzyMatchPaper(paperFilter, q.paper || '');
          const matchSubject = fuzzyMatchSubject(subjectFilter, q.subject || '');
          const matchSec = fuzzyMatchStandard(sectionFilter, getQuestionSection(q));
          const matchMicro = fuzzyMatchStandard(microtopicFilter, getQuestionMicro(q));
    ...
    if (activeCategoryItems && activeCategoryItems.length > 0) {
      activeCategoryItems.forEach(item => {
        getItemPaths(item).forEach(path => {
          ...
          const matchSubject = fuzzyMatchSubject(subjectFilter, path.subject);
          const matchSection = fuzzyMatchStandard(sectionFilter, path.sectionGroup || '');
          const matchMicro = fuzzyMatchStandard(microtopicFilter, path.microtopic || '');
          if (matchPaper && matchSubject && matchSection && matchMicro) {
            if (path.subtopic && fuzzyMatchStandard(subtopicFilter, path.subtopic)) {
      ...
      questions?.forEach(q => {
        if (q) {
          ...
          const matchSec = fuzzyMatchStandard(sectionFilter, getQuestionSection(q));
          const matchMicro = fuzzyMatchStandard(microtopicFilter, getQuestionMicro(q));
          const matchSub = fuzzyMatchStandard(subtopicFilter, getQuestionSub(q));
    ...
    if (activeCategoryItems && activeCategoryItems.length > 0) {
      activeCategoryItems.forEach(item => {
        getItemPaths(item).forEach(path => {
          const matchPaper = fuzzyMatchPaper(paperFilter, path.paper);
          const matchSubject = fuzzyMatchSubject(subjectFilter, path.subject);
          const matchSec = fuzzyMatchStandard(sectionFilter, path.sectionGroup || '');
          const matchMicro = fuzzyMatchStandard(microtopicFilter, path.microtopic || '');
          const matchSub = fuzzyMatchStandard(subtopicFilter, path.subtopic || '');
  ...
  useEffect(() => {
    ...
    filterTimerRef.current = setTimeout(() => {
      ...
      const valAddResult = valueAddItems.filter(va => {
        ...
        // If the user unchecks 'Value Additions' in the search filter, completely hide them globally
            ...
            renderItem={({ item, index }: { item: any; index: number }) => {
              ...
              return (
                <MainsQuestionCard
                  ...
                  selectedInstitute={selectedInstitutes[q.id]}
                  onSelectInstitute={(inst) => setSelectedInstitutes(prev => ({ ...prev, [q.id]: inst }))}
                  isBookmarked={savedIds.includes(q.id)}
                  onToggleBookmark={onToggleSaved}
                  onOpenDetailed={onOpenDetailed}
                  onCopyQuestion={(targetQ) => setCopyModalQuestion(targetQ)}
...
function MainsAISearchView({
  ...
  const runMainsSearch = async (overrideQuery?: string, overrideFilters?: MainsFilters, overrideEngineMode?: 'AI' | 'AI+Fuzzy' | 'Matching' | 'Exact') => {
    ...
    if (!overrideQuery) {
      ...
      // Fire-and-forget AsyncStorage save outside of the updater
      AsyncStorage.getItem('mains_search_history').then(existing => {
        const prev = existing ? JSON.parse(existing) : [];
        const next = [currentQuery, ...prev.filter((q: string) => q !== currentQuery)].slice(0, 10);
