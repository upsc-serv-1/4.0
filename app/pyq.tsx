...
import { InlineQuestionsViewer } from '../src/components/pyq/InlineQuestionsViewer';
...
function StickyHeatmapTable({
  ...
  onQuickViewPress?: (rowLabel: string) => void;
}) {
                ...
                {rows.map((row) => (
                  ...
                  > 
                    <View style={styles.heatmapLabelRow}>
                      ...
                      {onQuickViewPress && (
                        <TouchableOpacity 
                          onPress={() => onQuickViewPress(row.label)}
                          style={[styles.labelActionBtn, { marginLeft: 0, paddingHorizontal: 4 }]}
                          hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
                          activeOpacity={0.6}
                        >
                          <Eye size={14} color={colors.primary || '#7c3aed'} />
                        </TouchableOpacity>
                      )}
                      {onLabelActionPress && (
                        <TouchableOpacity 
...
export default function PyqAnalysisTab({ isEmbedded }: { isEmbedded?: boolean }) {
  ...
  const [inlineQuestionsFilter, setInlineQuestionsFilter] = useState<{
    type: 'section' | 'microtopic' | 'subtopic' | 'nanotopic';
    label: string;
    opts: any;
  } | null>(null);
  ...
  const navigateToLearning = (opts: { subject?: string; subjects?: string[]; section?: string; sections?: string[]; micro?: string; micros?: string[]; subtopic?: string; nanotopic?: string; year?: string; mode?: 'learning' | 'exam' | 'choice' }) => {
    ...
    if (isMainsStage) {
      ...
      if (opts.subtopic) engineParams.subtopic = opts.subtopic;
      if (opts.nanotopic) engineParams.nanotopic = opts.nanotopic;
          ...
          >
            ...
            <View style={{ gap: 24 }}>
              ...
              <StickyHeatmapTable
                ...
                onQuickViewPress={(sg) => {
                  Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                  if (inlineQuestionsFilter?.type === 'section' && inlineQuestionsFilter?.label === sg) {
                    setInlineQuestionsFilter(null);
                  } else {
                    setInlineQuestionsFilter({ type: 'section', label: sg, opts: { subject: pilotSubject, section: sg } });
                  }
                }}
                onYearPress={(year) => {
                  Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
              ...
              />
              {inlineQuestionsFilter?.type === 'section' && (
                <InlineQuestionsViewer 
                  opts={inlineQuestionsFilter.opts} 
                  label={inlineQuestionsFilter.label}
                  rawQuestions={rawQuestions}
                  onClose={() => setInlineQuestionsFilter(null)}
                />
              )}
              ...
              >
                <StickyHeatmapTable
                  ...
                  onQuickViewPress={(mic) => {
                    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                    if (inlineQuestionsFilter?.type === 'microtopic' && inlineQuestionsFilter?.label === mic) {
                      setInlineQuestionsFilter(null);
                    } else {
                      setInlineQuestionsFilter({ type: 'microtopic', label: mic, opts: { subject: pilotSubject, section: pilotSectionGroup || undefined, micro: mic } });
                    }
                  }}
                  onYearPress={(year) => {
                    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                ...
                />
                {inlineQuestionsFilter?.type === 'microtopic' && (
                  <InlineQuestionsViewer 
                    opts={inlineQuestionsFilter.opts} 
                    label={inlineQuestionsFilter.label}
                    rawQuestions={rawQuestions}
              ...
              {!isPrelims && (
                ...
                <StickyHeatmapTable
                  ...
                  onQuickViewPress={(sub) => {
                    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                    if (inlineQuestionsFilter?.type === 'subtopic' && inlineQuestionsFilter?.label === sub) {
                      setInlineQuestionsFilter(null);
                    } else {
                      setInlineQuestionsFilter({ type: 'subtopic', label: sub, opts: { subject: pilotSubject, section: pilotSectionGroup || undefined, micro: pilotMicro || undefined, subtopic: sub } });
                    }
                  }}
                  onYearPress={(year) => {
                    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                ...
                />
                {inlineQuestionsFilter?.type === 'subtopic' && (
                  <InlineQuestionsViewer 
                    opts={inlineQuestionsFilter.opts} 
                    label={inlineQuestionsFilter.label}
                    rawQuestions={rawQuestions}
              ...
              {isOptional && (
                ...
                <StickyHeatmapTable
                  ...
                  onQuickViewPress={(nano) => {
                    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                    if (inlineQuestionsFilter?.type === 'nanotopic' && inlineQuestionsFilter?.label === nano) {
                      setInlineQuestionsFilter(null);
                    } else {
                      setInlineQuestionsFilter({ type: 'nanotopic', label: nano, opts: { subject: pilotSubject, section: pilotSectionGroup || undefined, micro: pilotMicro || undefined, subtopic: pilotSubtopic || undefined, nanotopic: nano } });
                    }
                  }}
                  onYearPress={(year) => {
                    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                ...
                />
                {inlineQuestionsFilter?.type === 'nanotopic' && (
                  <InlineQuestionsViewer 
                    opts={inlineQuestionsFilter.opts} 
                    label={inlineQuestionsFilter.label}
                    rawQuestions={rawQuestions}
...
const styles = StyleSheet.create({
  ...
  modalContent: { borderRadius: 24, padding: 24, maxHeight: '80%', width: '100%', maxWidth: 480, borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)', shadowColor: '#000', shadowOffset: { width: 0, height: 16 }, shadowOpacity: 0.4, shadowRadius: 30, elevation: 12 },
