                ...
                <View key={topic} style={[s.topicRow, { borderBottomColor: colors.border, paddingVertical: 10 }]}>
                  ...
                  {trackingMethod === 'multi' && (
                    <View style={s.statusGrid}>
                      <StatusBtn active={itemProgress.ncert} onPress={() => toggleStatus(itemPath, 'ncert')} label={mode === 'prelims' ? "NCERT" : "Answer Written"} colors={colors} />
                      <StatusBtn active={itemProgress.pyqs} onPress={() => toggleStatus(itemPath, 'pyqs')} label="PYQ" colors={colors} />
                      <StatusBtn active={itemProgress.books} onPress={() => toggleStatus(itemPath, 'books')} label={mode === 'prelims' ? "Book" : "Test Series"} colors={colors} />
                    </View>
                  )}
                </View>
              );
...
function SyllabusTracker() {
  ...
  useEffect(() => {
    if (params.defaultMode) {
      setMode(params.defaultMode as Mode);
    }
  }, [params.defaultMode]);
  ...
  const [selectedSubject, setSelectedSubject] = useState<string | null>(params.subject || null);
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      AsyncStorage.getItem('syllabus_tracking_method').then(val => {
        if (val === 'single' || val === 'multi') setTrackingMethod(val);
      });
      AsyncStorage.getItem('syllabus_coverage_mode').then(val => {
        if (val === 'equal' || val === 'weighted') setCoverageMode(val);
      });
    }, [])
  );
          ...
          <View style={[s.topControlGrid, { marginHorizontal: spacing.lg }]}>
            <View style={[s.topLeftPanel, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              <View style={[s.tabBar, { marginHorizontal: 0, marginBottom: 8, backgroundColor: colors.bg, borderColor: colors.border }]}>
                <TouchableOpacity style={[s.tab, trackingMethod === 'single' && { backgroundColor: colors.primary }]} onPress={() => { setTrackingMethod('single'); AsyncStorage.setItem('syllabus_tracking_method', 'single'); }}>
                  <Text style={[s.tabText, { color: trackingMethod === 'single' ? '#fff' : colors.textSecondary }]}>Single</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[s.tab, trackingMethod === 'multi' && { backgroundColor: colors.primary }]} onPress={() => { setTrackingMethod('multi'); AsyncStorage.setItem('syllabus_tracking_method', 'multi'); }}>
              ...
              <View style={[s.tabBar, { marginHorizontal: 0, marginBottom: 0, backgroundColor: colors.bg, borderColor: colors.border }]}>
                <TouchableOpacity style={[s.tab, coverageMode === 'equal' && { backgroundColor: colors.primary }]} onPress={() => { setCoverageMode('equal'); AsyncStorage.setItem('syllabus_coverage_mode', 'equal'); }}>
                  <Text style={[s.tabText, { color: coverageMode === 'equal' ? '#fff' : colors.textSecondary }]}>Normal</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[s.tab, coverageMode === 'weighted' && { backgroundColor: colors.primary }]} onPress={() => { setCoverageMode('weighted'); AsyncStorage.setItem('syllabus_coverage_mode', 'weighted'); }}>
