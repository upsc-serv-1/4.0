...
export function normalizeKey(key: string): string {
  ...
  const overrides: Record<string, string> = {
    ...
    'date_time_of_surgery': 'surgery_date_time',
    'surgery_date_time': 'surgery_date_time',
    ...
    if (normalizedFlatData[normTarget] !== undefined) {
      ...
      if (typeof defaultCaseState[targetKey] === 'boolean') {
        ...
        if (typeof defaultCaseState[targetKey] === 'number') {
          ...
          // Handle datetime-local inputs: if format is YYYY-MM-DD (10 chars), append T00:00
          const datetimeKeys = ['date_of_admission', 'date_of_delivery', 'surgery_date_time'];
          if (datetimeKeys.includes(targetKey) && /^\d{4}-\d{2}-\d{2}$/.test(strVal)) {
            strVal = `${strVal}T00:00`;
          }
          val = strVal;
        }
