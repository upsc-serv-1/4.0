...
export function CaseForm({ initialData, onSave, onClear, isSaving }: CaseFormProps) {
  ...
  }, [formData.lmp, formData.edd]);

  // Auto calculate LMP from EDD (EDD - 280 days) - Reverse Naegele's rule
  useEffect(() => {
    if (formData.edd && !formData.lmp) {
      try {
        ...
        console.warn("Could not calculate LMP from EDD", err);
      }
    }
  }, [formData.edd, formData.lmp]);

  // Auto calculate Period of Gestation from LMP + Date of Delivery (DOD)
  // Formula: (Date of Delivery - LMP) / 7 = weeks (standard obstetric formula)
  useEffect(() => {
    if (formData.lmp && !formData.gestation_weeks) {
      if (formData.date_of_delivery) {
        try {
          const lmpDate = new Date(formData.lmp);
          const deliveryDate = new Date(formData.date_of_delivery);
          if (!isNaN(lmpDate.getTime()) && !isNaN(deliveryDate.getTime())) {
            ...
            if (diffDays > 0) {
              ...
              const gestStr = days > 0 ? `${weeks}+${days}` : `${weeks}`;
              setFormData((prev: CaseData) => ({
                ...prev,
                gestation_weeks: gestStr
              }));
            }
          }
        } catch (err) {
          console.warn("Could not calculate gestation weeks from DOD", err);
        }
      }
    }
  }, [formData.lmp, formData.date_of_delivery, formData.gestation_weeks]);
  ...
  const getFieldWarnings = () => {
    ...
    if (formData.gestation_weeks !== '') {
      const weeksStr = String(formData.gestation_weeks).split('+')[0];
      const gest = Number(weeksStr);
      ...
      <div className="tab-content-panel">
        ...
        {activeTab === 'demographics' && (
          <div className="form-section-grid">
            ...
            <div className="form-field">
              <label>Date of Admission (DOA)</label>
              <input type="date" name="date_of_admission" value={formData.date_of_admission} onChange={handleChange} />
            </div>

            <div className="form-field">
              <label>Time of Admission (TOA)</label>
              <input type="time" name="time_of_admission" value={formData.time_of_admission} onChange={handleChange} />
            </div>

            <div className="form-field">
              <label>Date of Delivery (DOD)</label>
              <input type="date" name="date_of_delivery" value={formData.date_of_delivery} onChange={handleChange} />
            </div>

            <div className="form-field">
              <label>Time of Delivery (TOD)</label>
              <input type="time" name="time_of_delivery" value={formData.time_of_delivery} onChange={handleChange} />
            </div>
            ...
            <div className="form-field">
              <label className={fieldWarnings.gestation_weeks ? "text-red-danger font-semibold" : ""}>
                Period of Gestation (weeks) {fieldWarnings.gestation_weeks && "⚠️"}
                {formData.lmp && formData.date_of_delivery && (
                  <span style={{ fontSize: '0.75rem', color: 'var(--text-accent)', marginLeft: '8px', fontWeight: 500 }}>🧮 Auto-calculated from LMP + DOD</span>
                )}
              </label>
              <input
                type="text"
                ...
                onChange={handleChange}
                placeholder="e.g. 38+3 (weeks+days)"
                className={fieldWarnings.gestation_weeks ? "input-warning" : ""}
                style={formData.lmp && formData.date_of_delivery ? { background: 'rgba(99,102,241,0.08)', borderColor: 'var(--accent)', fontWeight: 600 } : {}}
              />
            </div>
