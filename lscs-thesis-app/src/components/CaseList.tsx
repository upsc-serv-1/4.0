...
export function CaseList({ cases, onSelect, onDelete, isLoading }: CaseListProps) {
  ...
  // Reset current page when searching to prevent page mismatch
  ...
  const currentRecords = filteredCases.slice(indexOfFirstRecord, indexOfLastRecord);

  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setSearchTerm(e.target.value);
    setCurrentPage(1); // Reset page on keypress
  };
    ...
    <div className="card list-card">
      <div className="card-header list-header">
        <div>
          ...
          <p className="card-subtitle">Showing {totalRecords} cases stored in your Supabase database</p>
        </div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="pagination-bar" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '20px', borderTop: '1px solid var(--border-color)', paddingTop: '16px', flexWrap: 'wrap', gap: '12px' }}>
            <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
              Showing <strong>{indexOfFirstRecord + 1}</strong> to <strong>{Math.min(indexOfLastRecord, totalRecords)}</strong> of <strong>{totalRecords}</strong> cases
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.85rem' }}>
                ...
                <select 
                  ...
                  style={{ background: 'var(--bg-input)', color: 'var(--text-main)', border: '1px solid var(--border-color)', borderRadius: '6px', padding: '4px 8px', outline: 'none', fontWeight: 600 }}
                >
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                <button
                  ...
                  style={{ padding: '6px 12px', borderRadius: '6px', fontSize: '0.8rem', minWidth: 'auto' }}
                >
                  « First
                </button>
                <button
                ...
                <span style={{ display: 'inline-flex', alignItems: 'center', padding: '0 8px', fontSize: '0.85rem', fontWeight: 600 }}>
