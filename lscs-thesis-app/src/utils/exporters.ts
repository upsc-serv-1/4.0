...
export function exportToExcel(cases: Record<string, any>[]) {
  ...
  const formattedRows = cases.map((c) => ({
    "ID": c.id,
    "Date Created": c.created_at ? new Date(c.created_at).toLocaleString() : '',
    "Case No": c.case_no || '',
    "Reg No": c.reg_no || '',
    ...
    "Date & Time of Admission (DOA)": c.date_of_admission ? new Date(c.date_of_admission).toLocaleString() : '',
    "Date & Time of Delivery (DOD)": c.date_of_delivery ? new Date(c.date_of_delivery).toLocaleString() : '',
    ...
    "Date & Time of Surgery": c.surgery_date_time ? new Date(c.surgery_date_time).toLocaleString() : '',
          ...
          rows: [
            ...
            new TableRow({
              children: [
                ...
                createCell(c.date_of_admission ? new Date(c.date_of_admission).toLocaleDateString() : 'N/A', "FFFFFF", false),
                createCell("DOD", "7A869A", true),
                createCell(c.date_of_delivery ? new Date(c.date_of_delivery).toLocaleDateString() : 'N/A', "FFFFFF", false),
              ],
            }),
            new TableRow({
              children: [
