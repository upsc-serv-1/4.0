/**
 * PencilAnnotationEngine ΓÇö Pilot V2 Step 5
 * ----------------------------------------
 * In-memory stroke tracker for the page-level Notability-style overlay.
 *
 * Responsibilities (Phase 3 gaps from PILOT_V2_GAPS.md):
 *   1. Palm rejection ΓÇö ignore non-stylus input when pencilOnly = true.
 *   2. Relative-scaled coordinates ΓÇö every captured point is normalised to
 *      0..1 of the page bounds so drawings never drift when the user
 *      zooms / pans / resizes the document.
 *   3. Undo / Redo ΓÇö operation stacks consumed by the toolbar.
 *   4. Single page-level continuous overlay ΓÇö one engine instance owns ALL
 *      strokes for the whole note (no per-block canvases ΓåÆ no lag).
 *
 ...
 *  on every active-stroke point ΓÇö critical for ink latency. */
...
/** Faster motion ΓåÆ thinner stroke (mimics flicked pen). */
...
export interface EngineConfig {
  /** Page width in pixels ΓÇö used to convert screen ΓåÆ relative coords. */
  pageWidth: number;
...
export class PencilAnnotationEngine {
  ...
  /** Last captured screen coords ΓÇö used to dedupe noisy gesture events. */
  private lastSampleAt = 0;
  /** Currently-active tool / color / width ΓÇö set by the toolbar. */
   ...
   *  stroke.  Does NOT fire listeners ΓÇö the caller is responsible for
   ...
   *  / replaceAll). Active-stroke updates DO NOT fire this listener ΓÇö that is
   ...
   *  during drawing ΓÇö only the Skia canvas should listen here. */
  ...
  startStroke(screenX: number, screenY: number, pressure = 0.5): void {
    ...
    // Fresh array ΓÇö DO NOT share references with the stroke object so we can
    ...
    // Active-only notify ΓÇö parents do not re-render here.
    this.notifyActive();
  }
  ...
  addPoint(screenX: number, screenY: number, pressure?: number): void {
    ...
    const now = Date.now();
    // 4ms throttle ΓÇö matches Soft Notes
    ...
    // is the single biggest cause of write lag on long strokes ΓÇö eliminating
    // it brings Pilot V2 ink to Soft Notes-grade latency.
    this.currentPoints.push(point);
    // No object spread, no new object ΓÇö the canvas reads the same reference
  ...
  endStroke(): PilotV2PencilStroke | null {
    ...
    // Optional shape recognition ΓÇö pen tool only, only when feature is on.
    ...
    const finished: PilotV2PencilStroke = {
      ...this.currentStroke,
      // Snapshot ΓÇö break the shared reference so future strokes can mutate
  ...
  private notify() {
    // Legacy listener ΓÇö fire all three so existing callers stay correct.
  ...
  /** Fire ONLY active-stroke listeners. Cheap ΓÇö used on every move event. */
   ...
   *  redo / replaceAll ΓÇö never during the live move loop. */
...
/* -------------------------------------------------------------------------- */
/* SVG path helper ΓÇö used by both PencilCanvas (Skia) and the export engine    */
/* -------------------------------------------------------------------------- */

/** Convert relative-space points ΓåÆ absolute-pixel SVG path with quadratic
