/**
 * PencilCanvas ΓÇö Pilot V2 Step 6 + Step 8 (lasso)
 * -----------------------------------------------
 ...
 * at all times ΓÇö exactly per Phase 3 spec in PILOT_V2_GAPS.md.
 *
...
interface Props {
  ...
  /** Active tool ΓÇö used to switch behaviour for lasso. */
  ...
  drawingMode: boolean;
  /** Called after every committed stroke ΓÇö host persists to MMKV/Supabase. */
   ...
   *  show/hide) ΓÇö the Notability-style word-tracking transform. */
...
export function PencilCanvas({
  ...
  // Lasso state ΓÇö polygon being drawn + active selection.
  ...
  // Skia path cache moved into <CommittedStrokesLayer> ΓÇö see bottom of file.
  ...
  // point (O(1)) and pass that string directly to <Path> ΓÇö Skia accepts both
  ...
  useEffect(() => {
    // PERSISTED listener ΓÇö refresh the committed-strokes layer ONLY when a
    ...
    // ACTIVE listener ΓÇö fires on EVERY move event. We rebuild the live SVG
  ...
  const drawGesture = useMemo(() => {
    // UI-thread throttle ΓÇö drop sub-frame events before incurring the
    // runOnJS bridge cost. 4 ms Γëê 240 Hz, plenty for ProMotion 120 Hz
  ...
  return (
    ...
    >
      <GestureDetector gesture={drawGesture}>
        <Animated.View style={{ width, height }}>
          <Canvas style={{ width, height }}>
            <Group>
              {/* Committed strokes ΓÇö memoised so they DO NOT re-render when
              ...
              {/* Render active stroke separately ΓÇö uses a string-path
      ...
      {/* Floating selection pill ΓÇö drag to move, tap delete */}
...
function SelectionPill({ bounds, width, height, count, onMove, onDelete, onDismiss }: SelectionPillProps) {
  ...
  const dragGesture = useMemo(() => {
    return Gesture.Pan()
      .onUpdate((e) => {
        'worklet';
        // Use RNGH's incremental delta (changeX/changeY) ΓÇö this is the
        ...
        // which DO NOT persist reliably across worklet invocations ΓÇö each
...
/*   replaced ΓÇö never on every active-stroke point update. This is what       */
...
interface CommittedLayerProps {
   ...
   *  changes (e.g. sidebar show/hide) ΓÇö exactly how Notability keeps strokes
...
const CommittedStrokesLayer = React.memo(function CommittedStrokesLayer({
  ...
  // Per-instance Skia path cache ΓÇö rebuilt only when this layer re-renders,
   ...
   *
   *  **Notability / GoodNotes approach ΓÇö word-tracking anchors:**
   *
   ...
   *    ΓÇó sidebar show / hide changing page width  (horizontal shift)
   *    ΓÇó block reorder changing page position      (vertical shift)
   *    ΓÇó zoom / resize                              (both axes)
   *
  ...
  const applyBlockOffset = (s: PilotV2PencilStroke): PilotV2PencilStroke => {
    ...
    // ΓöÇΓöÇ Full span-anchor reprojection (page-relative anchors) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    if (
      ...
      // ΓöÇΓöÇ Y-axis: use pageRelY when available (orientation-safe) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Legacy Y-only delta (pre-Step-9 anchors without span offsets) ΓöÇΓöÇ
    const dy = blockRect.y / height - s.anchor.blockOriginY;
    if (Math.abs(dy) < 0.002) return s; // < 0.2 % of page ΓÇö negligible
    const points = s.points.map(p => ({
      ...p,
