...
export function PilotV2AIChat({ isOtherPopupOpen, activeQuestion, onSaveResponse, externalOpenTrigger, onOpenVitaminEditor }: PilotV2AIChatProps) {
  ...
  // template using a newline before sending ΓÇö per user spec.
  ...
  const handleSend = async (textToSend?: string, isPillClick?: boolean) => {
    ...
    try {
      ...
      if (lowerText.includes('polity') || lowerText.includes('emergency')) {
        ...
        aiReply = "≡ƒÆí **Polity Core Concept**: The structural federation in India is built with a strong unitary bias (quasi-federal), allowing national interests to reign supreme in times of crisis.";
      } else if (lowerText.includes('wrong') || lowerText.includes('incorrect')) {
        aiReply = "Γ¥î **Distractor Analysis**: Other choices are incorrect because they refer to Article 356 (President's Rule) or Article 360 (Financial Emergency), whereas Article 352 governs National Emergency only.";
      } else if (lowerText.includes('example') || lowerText.includes('real-world')) {
        aiReply = "≡ƒîì **Historical Precedent**: The landmark S.R. Bommai v. Union of India (1994) case laid down strict guidelines to prevent arbitrary invocation of Article 356 President's Rule in the states.";
      } else if (lowerText.includes('elephant')) {
        aiReply = "≡ƒÉÿ **Elephant Heritage**: Elephants are highly intelligent, social mammals native to Asia and Africa. In India, they hold significant cultural heritage status and are protected under Schedule I of the Wildlife Protection Act, 1972.";
      }
  ...
  return (
    ...
    >
      ...
      <Animated.View style={fabStyle}>
        ...
        >
          <Text style={{ fontSize: 26, lineHeight: 32 }}>≡ƒºá</Text>
        </TouchableOpacity>
      </Animated.View>
      ...
      <Animated.View style={chatStyle}>
        ...
        {/* Input Bar ΓÇö Moved back to top per user request */}
        ...
        {/* Presets Bar ΓÇö Under the input */}
        <View style={[styles.presetsBar, { borderBottomColor: colors.border, borderBottomWidth: 1 }]}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.actionsGrid}>
            {templates.map((template) => (
              ...
              >
                <Text style={{ fontSize: 14 }}>{template.button_emoji || '≡ƒñû'}</Text>
