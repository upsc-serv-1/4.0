/**
 * Pilot V2 ΓÇö Dashboard
 *
 * Faithful port of the KM `Dashboard` component:
 *   ΓÇó Top bar: breadcrumb + search + "+ New" button
 *   ΓÇó Greeting block (Good Morning / Afternoon / Evening + sub-line)
 *   ΓÇó "Continue Studying" horizontal carousel (3 sample cards)
 *   ΓÇó "Pinned Notes" 2-col grid (4 sample cards)
 *
...
export function PilotV2Dashboard() {
  ...
  // Pinned tab ΓåÆ full grid (no slice). Home/other ΓåÆ compact preview (top 6).
  ...
  const handleRestore = async (noteItem: PilotV2Note) => {
    ...
    try {
      ...
      Alert.alert('Restored', `ΓÇ£${noteItem.title}ΓÇ¥ has been restored.`);
    } catch (e) {
  ...
  const handlePurge = (noteItem: PilotV2Note) => {
    Alert.alert(
      'Delete permanently?',
      `Are you sure you want to permanently delete ΓÇ£${noteItem.title}ΓÇ¥? This action cannot be undone.`,
      [
          ...
          <View style={styles.filterBadge}>
            ...
            {quickFilter !== 'trash' && (
              <TouchableOpacity
                ...
                hitSlop={6}
              >
                <Text style={{ fontSize: 11, color: '#5B4EFA', fontWeight: '700' }}>Clear Γ£ò</Text>
              </TouchableOpacity>
            )}
          </View>
        ) : null}
                    ...
                    >
                      <View style={{ flex: 1 }}>
                        ...
                        <Text style={{ color: colors.textTertiary, fontSize: 12, marginTop: 4 }}>
                          {item.subject || 'General'} ΓÇó {formatRelative(item.updated_at)}
                        </Text>
                      </View>
            ...
            <View style={{ marginBottom: 32 }}>
              <Text style={[styles.h1, { color: colors.textPrimary }]}>
                {isSubjectMode && activeSubject
                  ...
                  : `${greetingFor(new Date())}, Aspirant ≡ƒæï`}
              </Text>
              <Text style={[styles.sub, { color: colors.textSecondary }]}>
                {isSubjectMode && activeSubject
