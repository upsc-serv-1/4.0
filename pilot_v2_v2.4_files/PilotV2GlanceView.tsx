/**
 * Pilot V2 ΓÇö Glance View (Issue 12: Page Scaling & Zooming)
 *
 * Premium reading workspace with:
 *   ΓÇó Edge-to-edge content layout (no maxWidth constraint)
 *   ΓÇó Pinch-to-zoom: entire page scales (NOT individual text font sizes)
 *   ΓÇó Smooth pan when zoomed ΓÇö 2D movement with inertia feel
 *   ΓÇó Double-tap reset or header Reset button
 *   ΓÇó Unified page/canvas background ΓÇö no "white page on grey" separation
 *   ΓÇó Zoom level badge with smooth spring animations
 */
...
/* ΓöÇΓöÇΓöÇ demo data ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */

const DEMO_BLOCKS: PilotV2Block[] = [
  { id: 'd1', type: 'heading', level: 1, text: 'Article 14 ΓÇö Equality Before Law' },
  {
  ...
  { id: 'd11', type: 'bullet', text: 'State of West Bengal v. Anwar Ali Sarkar (1952) ΓÇö established the test for reasonable classification.' },
  { id: 'd12', type: 'bullet', text: 'E.P. Royappa v. State of Tamil Nadu (1974) ΓÇö equality as a basic feature of the Constitution.' },
  { id: 'd13', type: 'bullet', text: 'Maneka Gandhi v. Union of India (1978) ΓÇö Articles 14, 19 and 21 form a "golden triangle".' },
  {
    id: 'd14', type: 'bullet', text:
      'Indra Sawhney v. Union of India (1992) ΓÇö Mandal Commission case; reservations capped at 50% with creamy-layer carve-out.',
  },
];

/* ΓöÇΓöÇΓöÇ component ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
...
export function PilotV2GlanceView() {
  ...
  const title = note?.title ?? 'Article 14 ΓÇö Equality Before Law';
  ...
  /* ΓöÇΓöÇ TRULY fixed page width (GoodNotes / Notability approach) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  /* The content + drawing surface are laid out at a width computed ONCE on
   * first render and stored in a ref.  This width NEVER changes ΓÇö not on
   * orientation change, not on sidebar toggle.  This guarantees:
   *   ΓÇó Text never reflows ΓåÆ all block positions stay pixel-perfect
   *   ΓÇó Drawing coordinates remain invariant ΓåÆ strokes never shift
   *   ΓÇó Sidebar show/hide and orientation only affect centering
   *
   * On tablets the initial width is: screenWidth ΓêÆ sidebarWidth ΓêÆ padding.
   * On phones: screenWidth ΓêÆ padding.
   * In both cases the value is frozen at mount time.
   */
  const SIDEBAR_WIDTH = 320;
  const BODY_PADDING = 32;
  const isTablet = screenWidth >= 768;
  // useRef to hold the frozen width ΓÇö computed once, never updated
  ...
  /* ΓöÇΓöÇ Pencil overlay (Step 6) ΓÇö drawable EVERYWHERE in glance view ΓöÇΓöÇΓöÇ */
  const [paperSize, setPaperSize] = useState({ w: 1, h: 1 });
  /** Block layout map ΓÇö keyed by block.id, same shape as EditorView.
  ...
  /** Ref to the pencil engine ΓÇö used by persistGlanceStrokes to sync anchor
  ...
  /* ΓöÇΓöÇ zoom state ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  ...
  /* ΓöÇΓöÇ Draggable Pencil FAB ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  ...
  /* ΓöÇΓöÇ Draggable Pencil Toolbar ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  ...
  /* ΓöÇΓöÇ reset zoom (callable from JS and worklets) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  ...
  /* ΓöÇΓöÇ pinch gesture ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  ...
  /* ΓöÇΓöÇ pan gesture (active when zoomed, 2D panning) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  ...
  /* ΓöÇΓöÇ double-tap to reset zoom ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  ...
  /* ΓöÇΓöÇ compose all gestures ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ*/
  const composedGesture = Gesture.Race(
    doubleTapGesture,
    Gesture.Simultaneous(pinchGesture, panGesture),
  );

  /* ΓöÇΓöÇ animated page style ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ*/
  ...
  /* ΓöÇΓöÇ scroll memory ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  useEffect(() => {
  ...
  /* ΓöÇΓöÇ navigation ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  const handleBack = () => {
    resetZoom();
    dispatch({ type: 'SET_VIEW_MODE', payload: state.view.selectedSubtopic ? 'noteList' : 'dashboard' });
  };

  /* ΓöÇΓöÇ share / export ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  const blocksToPlainText = (): string => {
    return blocks
      .map(b => {
        switch (b.type) {
          case 'heading':   return `\n# ${b.text}\n`;
          case 'bullet':    return `ΓÇó ${b.text}`;
    ...
    try {
      if (Platform.OS === 'web') {
        if ((navigator as any)?.share) {
          ...
          Alert.alert('Copied to clipboard', 'Note content copied ΓÇö paste it anywhere.');
        }
        return;
      }
  ...
  return (
    ...
    >
      {/* ΓöÇΓöÇ Cute Floating back button ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      <TouchableOpacity
      ...
      {/* ΓöÇΓöÇ Minimal floating controls (no header band) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      <View
      ...
      {/* ΓöÇΓöÇ Scalable page canvas ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      {/*
          The outer View clips overflow.
          GestureDetector captures pinch + pan + double-tap.
          Animated.View holds the scale/translate transform ΓÇö this IS the "page".
      ...
      */}
        ...
        <GestureDetector gesture={composedGesture}>
          <Animated.View style={[styles.page, animatedPageStyle]}>
            <ScrollView
              ...
              bounces={!isZoomed}
            >
              {/* Fixed-width page container ΓÇö content + strokes never reflow */}
              ...
              >
                ...
                <Text style={[styles.eog, { color: colors.textTertiary }]}>ΓÇö End of Glance ΓÇö</Text>
      ...
      {/* ΓöÇΓöÇ Zoom help bar (shown when zoomed) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      {isZoomed && (
      ...
      {/* ΓöÇΓöÇ Pencil FAB (Step 6) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      ...
      {/* ΓöÇΓöÇ Unified Export sheet (single, replaces Upload-icon legacy export) ΓöÇΓöÇ */}
...
/* ΓöÇΓöÇΓöÇ Block renderer ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
    ...
    case 'bullet':
      return (
        <View style={bStyles.bulletRow}>
          <Text style={[bStyles.bulletDot, { color: colors.textPrimary }]}>ΓÇó</Text>
          <View style={{ flex: 1 }}>
            <RenderHtml
...
const styles = StyleSheet.create({
  ...
  /* Content body ΓÇö NOTE: no maxWidth, full-width reading */
  body: {
    paddingHorizontal: 16,
    paddingTop: 28,
  },
