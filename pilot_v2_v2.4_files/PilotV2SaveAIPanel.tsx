 ...
 *   ΓÇó Expandable height ΓÇö defaults to ~70% of sheet, can expand to ~95%.
 *   ΓÇó Prompt TextInput (top) ΓÇö user types a custom command e.g. "Convert to Hindi".
 *   ΓÇó Horizontal scrollable preset chips (small icons) under the input ΓÇö
 *     replaces the old "Running AI" tab.  Tap a preset ΓåÆ command is auto-sent.
 ...
 *   ΓÇó AI output is rendered inside the *full* RichNoteEditor + RichToolbar
 *     stack (same one used by the SaveSheet body) so the user can edit /
 *     re-format / copy the answer just like a normal Pilot note.
 *   ΓÇó "Insert into note" button ΓÇö pushes the (possibly edited) HTML into
...
type Props = {
  ...
  seedContext?: {
    ...
    /** Existing body the user has already typed ΓÇö sent as base context. */
    body?: string | null;
  };
};
    ...
    >
      ...
      <View style={[styles.promptRow, { borderBottomColor: colors.border }]}>
        <TextInput
          ...
          placeholder='Ask AI anything ΓÇö e.g. "Convert to Hindi"'
      ...
      <View style={[styles.presetsBar, { borderBottomColor: colors.border }]}>
        ...
        >
          {templates.map((t) => (
            ...
            >
              <Text style={styles.presetEmoji}>{t.button_emoji || '≡ƒñû'}</Text>
              <Text
      ...
      {/* Rich-editor output area ΓÇö full formatting toolbar copied as-is */}
...
/** Minimal markdown ΓåÆ HTML converter that keeps things readable inside the
