 ...
 *
 *   1. Pilot-V2 blocks ΓåÆ `ExportNoteBlock[]` adapter (heading levels, bullets,
 *      numbered, checklist, quote, code, highlight all map cleanly).
 *   2. A "Block types" chip-row filter ΓÇö toggle Headings / Bullets / Numbered /
 *      Checklist / Quote / Code / Paragraph / Highlight on/off in one tap.
 *   3. A "Choose blocks to export" tag-chip selector ΓÇö every block becomes a
 *      chip you can flip on/off; "Select all" / "Select none" shortcuts. By
 *      default ALL blocks are included.
 *   4. Pencil-stroke pass-through ΓÇö annotations are preserved as vectors via
 ...
 *
 * All three legacy export buttons in Pilot V2 (Editor ΓåÆ More menu ΓåÆ PDF /
 * Image / Markdown, GlanceView ΓåÆ Upload icon, the unused PilotV2ExportSheet)
...
// ---------- Pilot V2 ΓåÆ ExportNoteBlock adapter ---------------------------
...
const blockToText = (b: PilotV2Block): string => {
  if (b.type === 'checklist') return `${b.checked ? 'Γÿæ' : 'ΓÿÉ'}  ${b.text || ''}`.trim();
  if (b.type === 'numbered')  return `1. ${b.text || ''}`.trim();
  if (b.type === 'bullet')    return `ΓÇó ${b.text || ''}`.trim();
  if (b.type === 'quote')     return `Γ¥¥ ${b.text || ''} Γ¥₧`.trim();
...
interface Props {
  visible: boolean;
  onClose: () => void;
  /** Document title ΓÇö also seeds export title input. */
  title: string;
...
export const PilotV2UnifiedExport: React.FC<Props> = ({
  ...
  // Block-type filter ΓÇö all on by default
  ...
  // Per-block selection ΓÇö all selected by default
  const [selectedBlockIds, setSelectedBlockIds] = useState<Set<string>>(
    () => new Set(blocks.map((b) => b.id)),
  );

  // Step 18 ΓÇö toggle for "Include pencil annotations". Default ON when there
  ...
  // Filter blocks ΓåÆ adapted ExportNoteBlock[] for the engine
  const filteredBlocks = useMemo(
    () => blocks.filter((b) => activeTypes[b.type] && selectedBlockIds.has(b.id)),
    [blocks, activeTypes, selectedBlockIds],
  );

  // Step 19 ΓÇö re-anchor any legacy unanchored strokes against the CURRENT
  // (full) block list (not the filtered one ΓÇö we need the original order +
  ...
  const survivingStrokes = useMemo(() => {
    ...
    return reAnchored.filter((s) => {
      ...
      // page-level ΓÇö they have no host block, so they survive unconditionally.
  ...
  // canvasMatch fast path in remapStrokeForExport triggers (export canvas Γëê
  // editor canvas), giving pixel-perfect stroke positioning via raw relativeΓåÆ
  // absolute scaling ΓÇö exactly what the user sees on screen.
  ...
  // Pilot V2 sensible defaults ΓÇö pastel cyan headings, plain paper, PDF.
  ...
  // Step 21 ΓÇö hybrid payload.  When the toggle is ON and at least one
...
const PilotV2ExtraFilters: React.FC<ExtraProps> = ({
  ...
  // Friendly "block summary" ΓÇö first 32 chars of text (or fallback label)
  const summarize = (b: PilotV2Block): string => {
    const t = (b.text || '').trim().replace(/\s+/g, ' ');
    if (t) return t.length > 32 ? `${t.slice(0, 32)}ΓÇª` : t;
    if (b.imageBase64 || b.imageUri) return '[Image]';
    if (b.attachment?.name) return `≡ƒôÄ ${b.attachment.name}`;
  ...
  return (
    <View style={{ marginTop: 6 }}>
      {/* ΓöÇΓöÇ Pencil annotations toggle (Step 18) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      {hasStrokes ? (
        <View
      ...
      {/* ΓöÇΓöÇ High Fidelity Visual Snapshot Toggle ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      <View
      ...
      {/* ΓöÇΓöÇ Block-type chips ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      <Text
        style={{
      ...
      {/* ΓöÇΓöÇ Per-block selection chips ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
