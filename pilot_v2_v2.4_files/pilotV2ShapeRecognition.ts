/**
 * pilotV2ShapeRecognition ΓÇö quick shape snap heuristics for Pencil
 * ----------------------------------------------------------------
 ...
 *
 * Heuristics are intentionally simple and local ΓÇö they run on the JS thread
...
const looksLikeRectangle = (pts: PilotV2PencilPoint[], b: Bounds): boolean => {
  ...
  // 4 sharp turns with mostly axis-aligned segments ΓåÆ rectangle.
  let onEdge = 0;
...
const looksLikeArrow = (pts: PilotV2PencilPoint[], b: Bounds): boolean => {
  ...
  let off = 0;
  for (const p of pts) {
    // perpendicular distance to headΓåÆtail line
...
function snapToArrow(pts: PilotV2PencilPoint[]): PilotV2PencilPoint[] {
  const head = pts[0];
  const tail = pts[pts.length - 1];
  // Build arrowhead ΓÇö a chevron at the tail.
