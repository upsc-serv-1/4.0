/**
 * pilotV2SyncQueue ΓÇö background retry queue for unsynced Pilot V2 notes.
 * ---------------------------------------------------------------------
 ...
 *
 * It's a thin wrapper ΓÇö the heavy lifting is in pilotV2LocalStore + the
  ...
  try {
    ...
    for (const noteId of dirty) {
      ...
      try {
        ...
        // network error ΓåÆ keep dirty, will retry next tick
...
/** Public ΓÇö called whenever the user makes an edit. Best-effort flush + schedule. */
...
export const startPilotV2SyncQueue = (): (() => void) => {
  ...
  const unsub = NetInfo.addEventListener((state) => {
    ...
    if (next && !online) {
      // network came back ΓåÆ retry immediately
      retryDelay = 1000;
