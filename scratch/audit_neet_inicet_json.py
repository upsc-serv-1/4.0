...
inicet_dir = r"C:\Users\Dr. Yogesh\Desktop\mains\neet and upsc cms\ini cet"
neetpg_dir = r"C:\Users\Dr. Yogesh\Desktop\mains\neet and upsc cms\neet pg"

json_files = glob.glob(os.path.join(inicet_dir, "*.json")) + glob.glob(os.path.join(neetpg_dir, "*.json"))

print(f"Auditing {len(json_files)} NEET PG / INI-CET JSON files...")
...
for fpath in json_files:
    ...
    for idx, q in enumerate(questions):
        for field, expected_type in expected_q_fields.items():
            if field not in q:
                # Some fields can be null, check if field is present
                missing_q.add(field)
