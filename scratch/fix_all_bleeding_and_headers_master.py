...
bak1_path = os.path.join(dir_path, "Anthropology_PAPER_1_PYQs_20_25_Extracted - Backup_2026-07-28_v4.md")
bak2_path = os.path.join(dir_path, "Anthropology_PAPER_2_PYQs_20_25_Extracted - Backup_2026-07-28_v4.md")
...
def process_section_master_fixed(ma_body):
    if not ma_body:
        return "", ""
    ...
    donts_match = re.search(
        r'(?:[^\n]*?)(?:\*\*|__)?\s*Don\'?ts?\s*(?:\*\*|__)?\s*\n(.*?)(?=\n\s*(?:[#/\*_]|\s)*(?:ANSWER|Model Answer|Notes?)\b|\n\n\s*(?:\*\*|[A-Z])[^\n]*?\n|\Z)',
        text, re.DOTALL | re.I
    )
...
def process_pristine_backup_master_fixed(backup_path, target_path):
    ...
    for s in secs:
        ...
        if len(ma_split) > 1:
            ...
            formatted_sec_text, json_ans_text = process_section_master_fixed(ma_body)
...
def main():
    for pair in BACKUP_PAIRS:
        label = pair["label"]
        print(f"\n{'='*60}")
        print(f"  Fixing All Bleeding Sections for {label}")
        print(f"{'='*60}")

        extracted_entries = process_pristine_backup_master_fixed(pair["backup_md"], pair["target_md"])
    ...
    print("\nDone! All bleeding sections fixed across all 315 questions. (Supabase upload skipped per directive)")
