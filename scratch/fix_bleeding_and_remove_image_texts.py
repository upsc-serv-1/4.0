...
bak1_path = os.path.join(dir_path, "Anthropology_PAPER_1_PYQs_20_25_Extracted - Backup_2026-07-28.md")
bak2_path = os.path.join(dir_path, "Anthropology_PAPER_2_PYQs_20_25_Extracted - Backup_2026-07-28.md")
...
def process_section_fixed(ma_body):
    if not ma_body:
        return "", ""
    ...
    # ROBUST Don'ts regex that stops cleanly before ANY ANSWER or Model Answer heading!
...
def process_pristine_backup_fixed(backup_path, target_path):
    ...
    for s in secs:
        ...
        if len(ma_split) > 1:
            ...
            formatted_sec_text, json_ans_text = process_section_fixed(ma_body)
    ...
    print(f"  Generated {os.path.basename(target_path)} cleanly with fixed Don'ts & removed image texts ({os.path.getsize(target_path)} bytes)")
...
def main():
    for pair in BACKUP_PAIRS:
        label = pair["label"]
        print(f"\n{'='*60}")
        print(f"  Fixing Bleeding Don'ts & Image Texts for {label}")
        print(f"{'='*60}")

        extracted_entries = process_pristine_backup_fixed(pair["backup_md"], pair["target_md"])
    ...
    print("\nDone! All bleeding Don'ts fixed & image texts removed in local MD & JSON files. (Supabase upload skipped per directive)")
