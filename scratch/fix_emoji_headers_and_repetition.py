...
# 1. CREATE BACKUP OF CURRENT MD FILES FIRST
dir_path = r"C:\Users\Dr. Yogesh\Downloads\Telegram Desktop"
md1_path = os.path.join(dir_path, "Anthropology_PAPER_1_PYQs_20_25_Extracted.md")
md2_path = os.path.join(dir_path, "Anthropology_PAPER_2_PYQs_20_25_Extracted.md")

bak1_path = os.path.join(dir_path, "Anthropology_PAPER_1_PYQs_20_25_Extracted - Backup_2026-07-28_v3.md")
bak2_path = os.path.join(dir_path, "Anthropology_PAPER_2_PYQs_20_25_Extracted - Backup_2026-07-28_v3.md")

shutil.copyfile(md1_path, bak1_path)
shutil.copyfile(md2_path, bak2_path)
print(f"  [BACKUP CREATED] {os.path.basename(bak1_path)} ({os.path.getsize(bak1_path)} bytes)")
print(f"  [BACKUP CREATED] {os.path.basename(bak2_path)} ({os.path.getsize(bak2_path)} bytes)")
...
def process_section_emoji_fixed(ma_body):
    if not ma_body:
        return "", ""
    ...
    # Fully emoji-tolerant regex patterns
    aspects_match = re.search(
        r'(?:[^\n]*?)(?:\*\*|__)?\s*Aspects?\s+to\s+Take\s+in?to\s+Account(?:\*\*|__)?\s*\n(.*?)(?=\n\s*(?:[^\n]*?)(?:\*\*|__)?\s*Structure\s+to\s+Follow|\n\s*(?:[^\n]*?)(?:\*\*|__)?\s*Don\'?ts?|\n\s*#{1,4}\s+|\Z)',
        text, re.DOTALL | re.I
    )
    structure_match = re.search(
        r'(?:[^\n]*?)(?:\*\*|__)?\s*Structure\s+to\s+Follow(?:\*\*|__)?\s*\n(.*?)(?=\n\s*(?:[^\n]*?)(?:\*\*|__)?\s*Don\'?ts?|\n\s*#{1,4}\s+|\/\/|\Z)',
        text, re.DOTALL | re.I
    )
    donts_match = re.search(
        r'(?:[^\n]*?)(?:\*\*|__)?\s*Don\'?ts?\s*(?:\*\*|__)?\s*\n(.*?)(?=\n\s*(?:#{1,4}|\/\/|\*\*|__)?\s*(?:ANSWER|Model Answer|Notes?)\b|\Z)',
        text, re.DOTALL | re.I
    )
    ...
    # Clean headers, remove Introduction headers completely
    ...
    # Boxify Lévi-Strauss section if present in rest
    ...
    rest = levi_pattern.sub(boxify_levi, rest)

    # Remove image diagram texts from rest
    rest = re.sub(r'\n*\s*\*\*A SKETCHED COMPARISON:\s*SOANIAN\s*vs\.\s*ACHEULIAN\s*TOOL\s*TRADITIONS\*\*\s*\n+.*?(?=\n\n|\Z)', '', rest, flags=re.DOTALL | re.I)
    rest = re.sub(r'\n*\s*UNDERSTANDING\s*THICK\s*DESCRPTION\s*vs\.\s*THIN\s*DESCRIPTION.*?(?=\n\n|\Z)', '', rest, flags=re.DOTALL | re.I)
...
def process_pristine_backup_emoji_fixed(backup_path, target_path):
    ...
    for s in secs:
        ...
        if len(ma_split) > 1:
            ...
            formatted_sec_text, json_ans_text = process_section_emoji_fixed(ma_body)
    ...
    # Post-cleaning for any remaining OCR diagram text artifacts
    final_md = re.sub(r'UNDERSTANDING\s*THICK\s*DESCRPTION\s*vs\.\s*THIN\s*DESCRIPTION.*?(?=\n\n---\n|\Z)', '', final_md, flags=re.DOTALL | re.I)
    final_md = re.sub(r'\*\*A SKETCHED COMPARISON:\s*SOANIAN\s*vs\.\s*ACHEULIAN\s*TOOL\s*TRADITIONS\*\*.*?(?=\n\n---\n|\Z)', '', final_md, flags=re.DOTALL | re.I)

    with open(target_path, "w", encoding="utf-8") as f:
        f.write(final_md)

    print(f"  Generated {os.path.basename(target_path)} cleanly ({os.path.getsize(target_path)} bytes)")
...
def main():
    for pair in BACKUP_PAIRS:
        label = pair["label"]
        print(f"\n{'='*60}")
        print(f"  Fixing Emoji Prep Notes for {label}")
        print(f"{'='*60}")

        extracted_entries = process_pristine_backup_emoji_fixed(pair["backup_md"], pair["target_md"])
    ...
    print("\nDone! All emoji prep note repetition fixed in local MD & JSON files. (Supabase upload skipped per directive)")
