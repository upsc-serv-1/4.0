...
bak1_path = os.path.join(dir_path, "Anthropology_PAPER_1_PYQs_20_25_Extracted - Backup_2026-07-28_v2.md")
bak2_path = os.path.join(dir_path, "Anthropology_PAPER_2_PYQs_20_25_Extracted - Backup_2026-07-28_v2.md")
...
def process_section_paper2_fixed(ma_body):
    if not ma_body:
        return "", ""
    ...
    # Robust prep notes regex matching unbolded, bolded, or bulleted headers
...
def process_pristine_backup_paper2_fixed(backup_path, target_path):
    ...
    for s in secs:
        ...
        if len(ma_split) > 1:
            ...
            formatted_sec_text, json_ans_text = process_section_paper2_fixed(ma_body)
    ...
    print(f"  Generated {os.path.basename(target_path)} cleanly with fixed Paper 2 Don'ts ({os.path.getsize(target_path)} bytes)")
...
def main():
    for pair in BACKUP_PAIRS:
        label = pair["label"]
        print(f"\n{'='*60}")
        print(f"  Fixing Paper 2 Bleeding Sections for {label}")
        print(f"{'='*60}")

        extracted_entries = process_pristine_backup_paper2_fixed(pair["backup_md"], pair["target_md"])
    ...
    print("\nDone! All Paper 2 bleeding sections fixed in local MD & JSON files. (Supabase upload skipped per directive)")
