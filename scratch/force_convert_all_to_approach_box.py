...
def convert_any_prep_to_approach_box(text):
    if text.startswith('| **Approach:'):
        return text

    # Case A: 3-column Markdown Table
    table_pattern = re.compile(
        r'\|\s*\*\*Aspects?\s+to\s+Take\s+in?to\s+Account\*\*\s*\|\s*\*\*Structure\s+to\s+Follow\*\*\s*\|\s*\*\*Don\'?ts?\*\*\s*\|\s*\n'
        r'\|\s*---+\s*\|\s*---+\s*\|\s*---+\s*\|\s*\n'
        r'\|\s*(.*?)\s*\|\s*(.*?)\s*\|\s*(.*?)\s*\|\s*\n\n?',
    ...
    m_raw = raw_pattern.search(text)
    if m_raw:
        aspects_fmt = format_cell_lines(m_raw.group(1))
        structure_fmt = format_cell_lines(m_raw.group(2)) if m_raw.group(2) else ""
        donts_fmt = format_cell_lines(m_raw.group(3)) if m_raw.group(3) else ""
...
def main():
    ...
    for f_info in FILES:
        ...
        print(f"\n{'='*60}")
        print(f"  Converting ALL prep blocks in {label} ({filepath})")
        print(f"{'='*60}")
        ...
        for q in questions:
            ...
            for a in q.get("answers", []):
                ...
                new_txt = convert_any_prep_to_approach_box(txt)
                if new_txt != txt:
