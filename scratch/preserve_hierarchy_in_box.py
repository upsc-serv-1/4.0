...
def process_section_slash_answer(ma_body):
    if not ma_body:
        return "", ""
...
def process_pristine_backup_slash_answer(backup_path, target_path):
    ...
    for s in secs:
        ...
        if len(ma_split) > 1:
            ...
            formatted_sec_text, json_ans_text = process_section_slash_answer(ma_body)
    ...
    print(f"  Generated {os.path.basename(target_path)} with // ANSWER ({os.path.getsize(target_path)} bytes)")
...
def main():
    for pair in BACKUP_PAIRS:
        label = pair["label"]
        print(f"\n{'='*60}")
        print(f"  Formatting // ANSWER Header for {label}")
        print(f"{'='*60}")

        extracted_entries = process_pristine_backup_slash_answer(pair["backup_md"], pair["target_md"])
    ...
    print("\nDone! // ANSWER header updated in local MD & JSON files. (Supabase upload skipped per directive)")
