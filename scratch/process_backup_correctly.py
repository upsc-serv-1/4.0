...
def process_md_file(backup_path, target_path):
    ...
    sections = re.split(r'\n(?=##\s*Question|\*\*Question ID:\*\*|\*\*Question:\*\*|\Z)', raw_md)
    processed_secs = []
    extracted_entries = []
    ...
    for sec in sections:
        ...
        q_match = re.search(r'\*\*Question:\*\*\s*(.*?)(?=\*\*Year:\*\*|\*\*Marks:\*\*|\n\n|\Z)', sec, re.DOTALL)
        question_text = q_match.group(1).strip() if q_match else ""

        # Format header with Question X
        sec_clean = sec.strip()
        if not sec_clean.startswith("## Question"):
            sec_clean = f"## Question {q_counter}\n" + sec_clean

        ans_split = re.split(r'##\s*Model Answer\s*\n', sec_clean, flags=re.I)
        if len(ans_split) > 1:
            ...
            processed_secs.append(sec_clean + "\n\n---\n\n")
