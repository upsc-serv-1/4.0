...
def process_section_no_intro(ma_body):
    if not ma_body:
        return "", ""
    ...
    rest = re.sub(r'^(?:#{1,4}\s*)?(?:\*\*|__)?\s*(?:ANSWER|Model Answer|Introduction)\s*(?:\*\*|__)?\s*\n?', '', rest, flags=re.I).strip()
...
def process_pristine_backup_no_intro(backup_path, target_path):
    ...
    for s in secs:
        ...
        if len(ma_split) > 1:
            ...
            formatted_sec_text, json_ans_text = process_section_no_intro(ma_body)
    ...
    print(f"  Generated {os.path.basename(target_path)} without Introduction headers ({os.path.getsize(target_path)} bytes)")
...
def main():
    for pair in BACKUP_PAIRS:
        label = pair["label"]
        print(f"\n{'='*60}")
        print(f"  Removing Introduction headers for {label}")
        print(f"{'='*60}")

        extracted_entries = process_pristine_backup_no_intro(pair["backup_md"], pair["target_md"])
    ...
    print("\nDone! Local MD files and JSON files updated. (Supabase upload skipped per directive)")
