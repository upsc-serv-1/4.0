...
def process_exact_md(backup_path, target_path):
    with open(backup_path, "r", encoding="utf-8") as f:
        raw_md = f.read()

    # Replace each Model Answer section in-place
    ...
    for i in range(1, len(parts), 2):
        ...
        # Content goes up to the next question heading or end of file
        # Find where next question starts
        q_next_m = re.search(r'\n(?=##\s*Question|\*\*Question ID:\*\*|\*\*Question:\*\*|\Z)', content)
        if q_next_m:
            ans_body = content[:q_next_m.start()]
            rest_of_section = content[q_next_m.start():]
        else:
            ans_body = content
            rest_of_section = ""

        # Find question_text or q_id from preceding part
    ...
    print(f"  Exact Processed {os.path.basename(backup_path)} -> {os.path.basename(target_path)} ({os.path.getsize(target_path)} bytes, {len(extracted_entries)} answers)")
...
def main():
    ...
    for pair in BACKUP_PAIRS:
        label = pair["label"]
        print(f"\n{'='*60}")
        print(f"  Exact In-Place Formatting for {label}")
        print(f"{'='*60}")

        extracted_entries = process_exact_md(pair["backup_md"], pair["target_md"])
