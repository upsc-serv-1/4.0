...
def process_section_hash_answer(ma_body):
    if not ma_body:
        return "", ""
...
def process_pristine_backup_hash_answer(backup_path, target_path):
    ...
    for s in secs:
        ...
        if len(ma_split) > 1:
            ...
            formatted_sec_text, json_ans_text = process_section_hash_answer(ma_body)
...
def main():
    for pair in BACKUP_PAIRS:
        label = pair["label"]
        print(f"\n{'='*60}")
        print(f"  Restoring ## ANSWER Header for {label}")
        print(f"{'='*60}")

        extracted_entries = process_pristine_backup_hash_answer(pair["backup_md"], pair["target_md"])
    ...
    print("\nDone! ## ANSWER header restored in local MD & JSON files. (Supabase upload skipped per directive)")
