...
def delete_existing_anthro_records():
    print("Deleting existing Anthropology records from Supabase...")
    # Delete answers for anthro questions first
    for prefix in ["mains-anthro1", "mains-anthro2"]:
        url_ans = f"{SUPABASE_URL}/rest/v1/mains_answers?question_id=like.{prefix}*"
        res = requests.delete(url_ans, headers=DELETE_HEADERS)
        print(f"  [DELETE ANSWERS] {prefix}: status {res.status_code}")

        url_q = f"{SUPABASE_URL}/rest/v1/mains_questions?id=like.{prefix}*"
        res_q = requests.delete(url_q, headers=DELETE_HEADERS)
        print(f"  [DELETE QUESTIONS] {prefix}: status {res_q.status_code}")
        ...
        for attempt in range(5):
            try:
                ...
                if resp.status_code in [200, 201]:
                    ...
                    print(f"  [WARNING] Batch {i//batch_size + 1} returned {resp.status_code}: {resp.text}. Retrying...")
                    time.sleep(3)
...
def main():
    delete_existing_anthro_records()
    ...
    print("\n[COMPLETE] Successfully synced Anthropology Paper 1 and Paper 2 to Supabase!")
