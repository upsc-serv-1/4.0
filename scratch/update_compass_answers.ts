...
async function run() {
  console.log('1. Fetching all Anthropology questions...');
  ...
  console.log(`Found ${qIds.length} Anthropology questions.`);
  
  console.log('2. Deleting old Compass answers for Anthropology...');
  const batchSize = 100;
  let deletedCount = 0;
  for (let i = 0; i < qIds.length; i += batchSize) {
    const chunk = qIds.slice(i, i + batchSize);
    
    const { data, error } = await supabase.from('mains_answers')
      .delete()
      .ilike('institute', 'Compass%')
  ...
  console.log('Finished deleting old Compass answers.');

  console.log('3. Importing new JSON files...');
  const files = [
    'C:\\Users\\Dr. Yogesh\\Downloads\\compass antrho\\final 3\\mains-upsc_anthro_paper_1_2012-2025_compass_updated_v3.json',
    'C:\\Users\\Dr. Yogesh\\Downloads\\compass antrho\\final 3\\mains-upsc_anthro_paper_2_2012-2025_compass_updated_v3.json'
  ];
