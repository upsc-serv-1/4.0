        ...
        for attempt in range(5):
            try:
                ...
                if resp.status_code in [200, 201]:
                    ...
                    success = True
                    print(f"  Batch {i//batch_size + 1}/{total_batches} -> {table_name}: OK ({len(batch)} rows)")
                    break
                else:
                    print(f"  [WARNING] Batch {i//batch_size + 1} returned {resp.status_code}: {resp.text[:200]}. Retrying in 5s...")
                    time.sleep(5)
...
def main():
    ...
    if all_questions:
        print("\n--- Uploading Questions (public.mains_questions) ---")
        upload_batch("mains_questions", all_questions)
        
    if all_answers:
        print("\n--- Uploading Answers (public.mains_answers) ---")
