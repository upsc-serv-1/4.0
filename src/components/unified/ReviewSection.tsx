...
interface ReviewSectionProps {
  ...
  onExternalTagUpdate?: (id: string, cat: string) => void;
  // Pre-computed data to avoid redundant fetching
  preComputedScoreData?: any;
  preComputedQuestions?: any[];
  preComputedHierarchy?: any;
  preComputedConfidence?: any[];
}
...
export const ReviewSection = ({ 
  ...
  externalTags, 
  onExternalTagUpdate,
  preComputedScoreData,
  preComputedQuestions,
  preComputedHierarchy,
  preComputedConfidence
}: ReviewSectionProps) => {
  const { colors } = useTheme();
  const { session } = useAuth();
  
  // Only use the hook if pre-computed data isn't provided
  const hookData = useSingleTestAnalytics(preComputedQuestions ? null : testAttemptId);
  
  const loading = preComputedQuestions ? false : hookData.loading;
  const error = preComputedQuestions ? null : hookData.error;
  const scoreData = preComputedScoreData || hookData.scoreData;
  const questions = preComputedQuestions || hookData.questions;
  const hierarchicalPerformance = preComputedHierarchy || hookData.hierarchicalPerformance;
  const confidenceMetrics = preComputedConfidence || hookData.confidenceMetrics;
  const testId = hookData.testId; // testId usually comes from the attempt fetch

  const [sectionOrder, setSectionOrder] = useState<string[]>(['advanced_link', ...DEFAULT_ANALYTICS_LAYOUT.review]);
  const [localTags, setLocalTags] = useState<Record<string, string>>({});

  // Sync initial tags from questions
  useEffect(() => {
    if (questions) {
      const initialTags: Record<string, string> = {};
      questions.forEach(q => {
        if (q.errorCategory) initialTags[q.id] = q.errorCategory;
      });
      setLocalTags(initialTags);
    }
  }, [questions]);

  // Merge external tags if provided
  const effectiveTags = externalTags || localTags;

  // Dynamically calculate error analytics for the chart
  const errorDonutData = React.useMemo(() => {
    if (!questions) return [];
    const counts: Record<string, number> = { 
      'Fact Mistake': 0, 'Concept Gap': 0, 'Silly Mistake': 0, 'Overthinking': 0, 'Skipped': 0 
    };
    
    questions.forEach(q => {
      const isIncorrect = q.selectedAnswer?.toLowerCase() !== q.correctAnswer?.toLowerCase() && !!q.selectedAnswer;
      if (isIncorrect) {
        const cat = effectiveTags[q.id] || q.errorCategory || 'Other';
  ...
  const handleTagError = async (questionId: string, errorType: string) => {
    ...
    if (onExternalTagUpdate) onExternalTagUpdate(questionId, errorType);
  ...
  const sectionBlocks: Record<string, React.ReactNode> = {
    ...
    mistake_types: errorDonutData.length > 0 ? (
      <View key="mistake_types" style={[styles.chartCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
        <View style={styles.cardHeader}>
          ...
          <Text style={[styles.cardTitle, { color: colors.textPrimary }]}>Mistake Types (Error DNA)</Text>
        </View>
        <Text style={[styles.cardSubtitle, { color: colors.textTertiary, marginBottom: 20 }]}>Categorization of your incorrect attempts</Text>
        <DonutChart 
          data={errorDonutData} 
          size={200} 
          colors={['#ef4444', '#f59e0b', '#fbbf24', '#3b82f6', colors.textTertiary]} 
          legendMode="arc"
          strokeWidth={20}
        />
      </View>
