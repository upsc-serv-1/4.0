...
export function useMainsTaggedVault(userId: string | undefined, localQuestionsList?: any[]) {
  ...
  const localQuestionsListRef = useRef(localQuestionsList);
  localQuestionsListRef.current = localQuestionsList;
  ...
  const fetchVaultData = useCallback(async () => {
    ...
    try {
      ...
      const qList = localQuestionsListRef.current;
      const questionsList = qList && qList.length > 0 
        ? qList 
