...
export interface AdvancedMetrics {
  ...
  confidence: Record<string, { total: number; correct: number; incorrect: number; accuracy: number }>;
}
...
export interface HierarchicalPerformance {
  ...
  overall: PerformanceStats & { advanced: AdvancedMetrics };
  pyq: PerformanceStats & { advanced: AdvancedMetrics };
  advanced: AdvancedMetrics;
}
...
const createBaseAdvanced = (): AdvancedMetrics => ({
  ...
  confidence: {
    '100% Sure': { total: 0, correct: 0, incorrect: 0, accuracy: 0 },
    'Logical Elimination': { total: 0, correct: 0, incorrect: 0, accuracy: 0 },
    'Pure Guess': { total: 0, correct: 0, incorrect: 0, accuracy: 0 },
    'UPSC Funda': { total: 0, correct: 0, incorrect: 0, accuracy: 0 },
    // Also support numeric indexing for legacy UI access
    '0': { total: 0, correct: 0, incorrect: 0, accuracy: 0 },
    '1': { total: 0, correct: 0, incorrect: 0, accuracy: 0 },
    '2': { total: 0, correct: 0, incorrect: 0, accuracy: 0 },
    '3': { total: 0, correct: 0, incorrect: 0, accuracy: 0 }
  }
});
...
export function buildHierarchicalPerformanceReport(questions: QuestionAttempt[]): HierarchicalPerformance {
  const report: HierarchicalPerformance = { 
    subjects: {},
    overall: { ...createBaseStats(), advanced: createBaseAdvanced() },
    pyq: { ...createBaseStats(), advanced: createBaseAdvanced() },
  ...
  questions.forEach((question) => {
    ...
    report.overall.timeSpent += question.timeSpentSeconds || 0;
    if (isCorrect) report.overall.correct += 1;
    if (isIncorrect) report.overall.incorrect += 1;
    if (isUnattempted) report.overall.unattempted += 1;

    // 0.1 PYQ Level
    if ((question as any).isPyq) {
      report.pyq.total += 1;
      report.pyq.timeSpent += question.timeSpentSeconds || 0;
      if (isCorrect) report.pyq.correct += 1;
      if (isIncorrect) report.pyq.incorrect += 1;
      if (isUnattempted) report.pyq.unattempted += 1;
    }
    ...
    const targets = [report.advanced, report.overall.advanced, subject.advanced, section.advanced];
    if ((question as any).isPyq) {
      targets.push(report.pyq.advanced);
    }
    ...
    targets.forEach(t => {
      ...
      if (question.confidence) {
        ...
        if (!t.confidence[normalizedConfidence]) {
          t.confidence[normalizedConfidence] = { total: 0, correct: 0, incorrect: 0, accuracy: 0 };
        }
        ...
        // Support numeric indexing for confidence (0: Sure, 1: Logical, 2: Guess, 3: Funda)
        const idxMap: Record<string, string> = {
          '100% Sure': '0',
          'Logical Elimination': '1',
          'Pure Guess': '2',
          'UPSC Funda': '3'
        };
        const idx = idxMap[normalizedConfidence];
        if (idx) {
          if (!t.confidence[idx]) t.confidence[idx] = { total: 0, correct: 0, incorrect: 0, accuracy: 0 };
          t.confidence[idx].total++;
          if (isCorrect) t.confidence[idx].correct++;
          if (isIncorrect) t.confidence[idx].incorrect++;
        }
      }
  ...
  const updateConfAcc = (adv: AdvancedMetrics) => {
    Object.values(adv.confidence).forEach(c => {
      c.accuracy = calculateAccuracy(c.correct, c.correct + c.incorrect);
    });
  };

  // Calculate Global Accuracies
  report.overall.accuracy = calculateAccuracy(report.overall.correct, report.overall.correct + report.overall.incorrect);
  updateConfAcc(report.overall.advanced);

  report.pyq.accuracy = calculateAccuracy(report.pyq.correct, report.pyq.correct + report.pyq.incorrect);
  updateConfAcc(report.pyq.advanced);

  updateConfAcc(report.advanced);
  ...
  for (const subjectKey in report.subjects) {
    ...
    updateConfAcc(subject.advanced);
    ...
    for (const sectionKey in subject.sectionGroups) {
      ...
      updateConfAcc(section.advanced);
