...
export const generateAnalyticsPdfHtml = ({
  ...
  const clamp = (v: number) => Math.max(0.25, Math.min(3, v));
  const marginTop = clamp(marginTopCm);
  const marginRight = clamp(marginRightCm);
  const marginBottom = clamp(marginBottomCm);
  const marginLeft = clamp(marginLeftCm);
  ...
  return `
    <html>
      <head>
        <meta charset="utf-8" />
        <style>
          @page { size: A4; margin: ${marginTop}cm ${marginRight}cm ${marginBottom}cm ${marginLeft}cm; }
          * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; color-adjust: exact !important; }
          html, body { margin: 0; padding: 0; }
          body { font-family: -apple-system, system-ui, BlinkMacSystemFont, Arial, sans-serif; color: #0f172a; line-height: 1.5; }
      ...
      <body>
        ...
        <div class="footer">
          Generated by Dr. UPSC Intelligence Engine &bull; Confidential Performance Report
        </div>
        </div> <!-- /page-content -->
      </body>
    </html>
